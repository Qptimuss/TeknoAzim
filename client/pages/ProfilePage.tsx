...
export default function ProfilePage() {
  ...
  const profileDetailsMutation = useMutation({
    ...
    onError: (error: Error) => {
      toast.error("Güncelleme Hatası", { description: error.message });
    },
  });
  ...
  const avatarUploadMutation = useMutation({
    ...
    onError: (error: Error) => toast.error("Profil fotoğrafı güncellenirken bir hata oluştu.", { description: error.message }),
  });
  ...
  const avatarDeleteMutation = useMutation({
    ...
    onError: (error: Error) => toast.error("Profil fotoğrafı silinirken bir hata oluştu.", { description: error.message }),
  });
  ...
  const postDeleteMutation = useMutation({
    ...
    onError: (error: Error) => toast.error("Blog yazısı silinirken bir hata oluştu.", { description: error.message }),
  ...
  const accountDeleteMutation = useMutation({
    mutationFn: async () => {
      ...
      if (sessionError || !session) throw new Error("Kimlik doğrulama oturumu bulunamadı.");
    ...
    onError: (error: Error) => toast.error("Hesap silinirken bir hata oluştu.", { description: error.message }),
  ...
  const handleTitleChange = (newTitle: string) => {
    profileDetailsMutation.mutate({ selected_title: newTitle === 'none' ? null : newTitle }, {
      onSuccess: () => toast.success('Ünvan güncellendi.'),
    });
  };

  const handleFrameSelect = (frameName: string) => {
    if (!user) return;
    const isRemoving = user.selected_frame === frameName;
    profileDetailsMutation.mutate({ selected_frame: isRemoving ? null : frameName }, {
      onSuccess: () => toast.success(isRemoving ? 'Çerçeve kaldırıldı.' : 'Çerçeve güncellendi!'),
    });
  };
  ...
  const handleNameSave = () => {
    ...
    profileDetailsMutation.mutate({ name: nameValue }, {
      onSuccess: () => {
        toast.success('İsim güncellendi!');
        setIsEditingName(false);
      },
      onError: () => setNameValue(user.name || ""),
    });
  };
  ...
  const handleDescriptionSave = () => {
    ...
    profileDetailsMutation.mutate({ description: descriptionValue }, {
      onSuccess: () => {
        toast.success('Açıklama güncellendi!');
        setIsEditingDescription(false);
      },
      onError: () => setDescriptionValue(user.description || ""),
    });
  };
  ...
  const handleCropComplete = (croppedBlob: Blob) => {
    ...
    avatarUploadMutation.mutate(croppedFile);
  };

  if (loading) return <div className="text-foreground text-center p-12">Yükleniyor...</div>;
  if (!user) return null;
  ...
  return (
    <>
      <div className="container mx-auto px-5 py-12">
        <h1 className="text-foreground text-4xl md:text-5xl font-outfit font-bold mb-8">Profilim</h1>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1">
            <div className="bg-card border border-border rounded-lg p-8">
              <div className="flex flex-col items-center mb-6 space-y-2 text-center">
                <div className="flex items-center justify-center gap-2">
                  <button onClick={() => user.avatar_url && setIsViewerOpen(true)} disabled={!user.avatar_url} className="disabled:cursor-default">
                    {user.selected_frame === 'Nova' ? (
                      <NovaFrame><AvatarPreview sizeClass="h-24 w-24" /></NovaFrame>
                    ) : (
                      <div className={cn("p-1", selectedFrame?.className)}><AvatarPreview sizeClass="h-24 w-24" /></div>
                    )}
                  </button>
                ...
                {user.selected_title && <p className="text-yellow-400 font-semibold text-sm flex items-center justify-center gap-1"><SelectedTitleIcon className="h-4 w-4" /> {user.selected_title}</p>}
              ...
              <div className="mb-6 border-t border-border pt-6">
                ...
                <TooltipProvider><Tooltip><TooltipTrigger className="w-full"><Progress value={expProgress} className="w-full" /></TooltipTrigger><TooltipContent><p>Toplam Deneyim: {user.exp || 0} EXP</p></TooltipContent></Tooltip></TooltipProvider>
                <div className="text-center text-sm text-muted-foreground mt-2">{`${expInCurrentLevel} / ${expForNextLevel} EXP`}</div>
              </div>
              <div className="mb-6 border-t border-border pt-6">
              ...
              <div className="mb-6 border-t border-border pt-6">
                <div className="flex justify-between items-center mb-4"><h3 className="text-card-foreground text-xl font-outfit font-bold">Rozetler</h3><span className="text-sm text-muted-foreground">{user.badges?.length || 0} / {ALL_BADGES.length}</span></div>
                  ...
                  {ALL_BADGES.map((badge) => {
                    ...
                    return (
                      <div key={badge.name} className="flex items-start gap-4">
                        <div className={cn("flex items-center justify-center bg-card p-3 rounded-full border border-border shrink-0", !hasBadge && "opacity-30 grayscale")}><Icon className="h-6 w-6 text-yellow-400" /></div>
                        <div><p className="font-bold text-foreground">{badge.name}</p><p className="text-sm text-muted-foreground">{badge.description}</p>{!hasBadge && <p className="text-xs text-red-400 mt-1">(Henüz kazanılmadı)</p>}</div>
                      </div>
                    );
                  })}
                </div>
                <p className="text-xs text-muted-foreground mt-2 text-center">Bir blog oluşturmak 25 EXP, bir rozet kazanmak 50 EXP verir.</p>
              </div>
          ...
          <div className="lg:col-span-2">
            ...
            {postsLoading ? <p className="text-muted-foreground">Bloglar yükleniyor...</p> : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {userPosts?.map(post => <BlogCard key={post.id} post={post} showDelete={true} onDelete={() => setPostToDelete({ id: post.id, imageUrl: post.image_url })} hideProfileLink={true} />)}
                <CreateBlogCard />
              </div>
            )}
        ...
        <AlertDialog open={!!postToDelete} onOpenChange={(open) => !open && setPostToDelete(null)}>
          <AlertDialogContent>
            <AlertDialogHeader><AlertDialogTitle>Blog Yazısını Silmek İstediğinize Emin Misiniz?</AlertDialogTitle><AlertDialogDescription>Bu işlem geri alınamaz. Blog yazınız, tüm yorumları ve oylarıyla birlikte kalıcı olarak silinecektir.<span className="font-bold text-destructive"> Ayrıca, bu gönderiden kazandığınız 25 EXP'yi kaybedeceksiniz.</span></AlertDialogDescription></AlertDialogHeader>
            <AlertDialogFooter><AlertDialogCancel>İptal</AlertDialogCancel><AlertDialogAction onClick={() => postToDelete && postDeleteMutation.mutate(postToDelete)} disabled={postDeleteMutation.isPending} className="bg-red-600 hover:bg-red-700 text-white">{postDeleteMutation.isPending ? "Siliniyor..." : "Sil"}</AlertDialogAction></AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
      ...
      <AlertDialog open={showDeleteAccountDialog} onOpenChange={setShowDeleteAccountDialog}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Hesabınızı Silmek İstediğinize Emin Misiniz?</AlertDialogTitle><AlertDialogDescription>Bu işlem geri alınamaz. Tüm blog yazılarınız, yorumlarınız ve profil verileriniz kalıcı olarak silinecektir. Bu işlemi onaylıyor musunuz?</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel disabled={accountDeleteMutation.isPending}>İptal</AlertDialogCancel><AlertDialogAction onClick={() => accountDeleteMutation.mutate()} disabled={accountDeleteMutation.isPending} className="bg-red-600 hover:bg-red-700 text-white">{accountDeleteMutation.isPending ? "Siliniyor..." : "Evet, Hesabımı Sil"}</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      <AlertDialog open={showDeleteAvatarDialog} onOpenChange={setShowDeleteAvatarDialog}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Profil Fotoğrafını Sil</AlertDialogTitle><AlertDialogDescription>Mevcut profil fotoğrafınızı kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter><AlertDialogCancel disabled={avatarDeleteMutation.isPending}>İptal</AlertDialogCancel><AlertDialogAction onClick={() => avatarDeleteMutation.mutate()} disabled={avatarDeleteMutation.isPending} className="bg-red-600 hover:bg-red-700 text-white">{avatarDeleteMutation.isPending ? "Siliniyor..." : "Sil"}</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      {user.avatar_url && <ImageViewerDialog open={isViewerOpen} onOpenChange={setIsViewerOpen} imageUrl={user.avatar_url} imageAlt={user.name || "Profil Fotoğrafı"} />}
    </>
  );
}