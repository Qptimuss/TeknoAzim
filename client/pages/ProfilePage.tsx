...
import { User as UserIcon, CheckCircle, Pencil, Check, X, Lock, Trash2, Loader2, Server } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleCheckEnv = async () => {
    ...
    try {
      const result = await fetchWithoutAuth('/api/check-env');
      setEnvCheckResult(result.variables);
      toast.success("Ortam değişkenleri başarıyla çekildi.");
    } catch (error) {
      toast.error("Ortam değişkenleri çekilemedi.", { description: error instanceof Error ? error.message : "Bilinmeyen hata." });
      setEnvCheckResult({ error: error instanceof Error ? error.message : "Bilinmeyen hata." });
    } finally {
            ...
            <div className="bg-card border border-border rounded-lg p-8">
              ...
              {isUserAdmin && (
                <div className="border-t border-border pt-6 mt-6">
                  <h3 className="text-primary text-xl font-outfit font-bold mb-4 flex items-center gap-2">
                    <Server className="h-5 w-5" /> Admin Araçları
                  </h3>
                  <Button 
                    variant="outline" 
                    className="w-full" 
                  ...
                  {envCheckResult && (
                    <div className="mt-4 p-3 bg-muted rounded-md text-xs break-all">
                      {envCheckResult.error ? (
                        <p className="text-red-500">Hata: {envCheckResult.error}</p>
                      ) : (
                        Object.entries(envCheckResult).map(([key, value]) => (
                          <div key={key} className="mb-1">
                            <span className="font-semibold text-foreground">{key}:</span>
                            <span className={cn("ml-2", (value as any).isSet ? "text-green-500" : "text-red-500")}>
                              {(value as any).isSet ? "AYARLI" : "AYARLANMADI"}
                            </span>
                            <p className="text-muted-foreground italic ml-4">Önizleme: {(value as any).preview}</p>
                            {key === "ADMIN_EMAILS" && (value as any).isSet && (
                              <p className="text-muted-foreground italic ml-4">Tam Değer: {(value as any).fullValue}</p>
                            )}
                          </div>
                        ))
                      )}
